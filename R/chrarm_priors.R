...
ApplyChrArmPrior <- function(mode.res, model.id, train.obj, verbose=FALSE) {
    ...
    for (j in seq_len(m)) {
      ...
      clust.ll <- DMultmix(mode.chrarm, clust.res, max.q, log.p=TRUE,
                           verbose=verbose)
      mode.res[["mode.tab"]][j, "clust_LL"] <- clust.ll  
...
GetChrArmMat <- function(mode.res, mode, max.q, n.arm= 39) {
  if (dim(mode.res[["chr.arm.tab"]])[2] == 2) {
    chrarm.mat <- rbind(mode.res[["chr.arm.tab"]][mode, 1, c(1:n.arm),],
                        mode.res[["chr.arm.tab"]][mode, 2, c(1:n.arm), ])
  } else {
    chrarm.mat <- mode.res[["chr.arm.tab"]][mode, 1, c(1:n.arm), ]
  }
