...
total_tx_data = function(error_model, d) {
  return(d) 
}

total_inv_tx_data = function(error_model, d) {
  return(d) 
}

total_atten = function(x, at) { 
  return(x) 
}

total_inv_atten = function(x, at) { 
  return(x) 
}
...
get_tcr_chr_arm_segs = function(seg.obj, chr_arm_dat) {
  ...
  for (i in 1:nrow(seg.dat)) {
    if (as.integer(seg.dat[i,"Chromosome"]) != as.integer(chr_arm_dat["chr"])) { 
      next 
    }
...
total_get_scna_stderr_grid_density = function(obs, grid, sigma_h, i=NA) {
  N = length(obs$d.tx)
  grid_dens = matrix(NA, nrow=N, ncol=length(grid))
  dx = c(0, diff(grid))
  
  if (is.na(i)) {
    for (i in seq_len(N)) {
      grid_dens[i, ] = dnorm(grid, obs$d.tx[i], obs$d.stderr[i] + sigma_h) * dx
    }
  } else {
    grid_dens = dnorm(grid, obs$d.tx[i], obs$d.stderr[i] + sigma_h) * dx
  }
...
total_get_abs_seg_dat = function(segobj, at) {
  Q = 15
  qq = Q
